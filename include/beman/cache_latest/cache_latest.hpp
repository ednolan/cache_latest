...
namespace beman {
    ...
    class non_propagating_cache
    {
        ...
        public:
            ...
            non_propagating_cache(non_propagating_cache&& other)
            {
                other.t_.reset();
            }
    ...
    namespace __cache_latest {
        ...
        struct __fn {
            template <std::ranges::input_range _View> requires std::ranges::view<_View>
            [[nodiscard]] constexpr auto
            operator()(_View&& _view) {
                return cache_latest_view(std::forward<_View>(_view));
            }

            [[nodiscard]] constexpr auto
            operator()() {
                return __pipeable(std::bind_back(*this));
            }

            // template<std::ranges::input_range R>
            // friend constexpr auto operator|(R&& r, __fn) {
            //     return cache_latest_fn{}(std::forward<R>(r));
            // }
        };
